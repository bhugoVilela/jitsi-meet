// This file is a fork of microphone-stream tweaked to accept 2 channels
...
export default class MicrophoneStream extends Readable {
  ...
  constructor(opts: MicrophoneStreamOptions = { objectMode: false }) {
    ...
    this.bufferSize = bufferSize || typeof window.AudioContext === 'undefined' ? 4096 : null;
    ...
    this.recorder = this.context.createScriptProcessor(bufferSize as unknown as number, inputChannels, outputChannels);
  ...
  public setStream(stream: MediaStream): void {
    ...
    const recorderProcess = (e: AudioProcessingEvent) => {
      if (this.recording) {
        if (this.objectMode) {
          this.push(e.inputBuffer); // Push the complete AudioBuffer in objectMode
        } else {
          ...
          this.push(Buffer.from(interleaved.buffer)); // Convert to buffer and push
        }
      }
    };
